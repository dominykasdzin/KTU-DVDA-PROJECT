...
loan_model <- h2o.loadModel("../4-model/gbm_model1")
...
ui <- dashboardPage(
  dashboardHeader(
    ...
    #    img(src = "D:/project/app/www/logo.jpg", height = '15px')
    #    ),
    #  class = 'dropdown'
    #)
  ),
  ...
  '))
    ...
    tabItems(
      ...
      tabItem(tabName = "model_info",
              fluidRow(
                ...
                box(title = "H2O Model Information", status = "info", solidHeader = T, collapsible = T,
    ...
    tags$footer(
      style = "position: fixed; bottom: 0; width: 100%; text-align: center; padding: 10px;",
      "Dominykas Dzingelevičius, 2023"
    )
  )
)
  ...
  observeEvent(input$submit, {
    ...
    output$classificationResult <- renderText({
      ...
      if (result == "Loan Denied") {
        ...
        #shinyjs::removeClass(selector = "body", class = "green-background blink")
        #shinyjs::runjs("document.getElementById('resultGif').src = 'www/rejected.gif';")
      } else {
        shinyjs::addClass(selector = "body", class = "green-background blink")
        #shinyjs::removeClass(selector = "body", class = "red-background blink")
        #shinyjs::runjs("document.getElementById('resultGif').src = 'www/approved.gif';")
      }
      return(result)
    })
